...
class MultiLabelClassifier(nn.Module):
    def __init__(self, attributes=None, n_attributes=0, n_shared_layers=0, hidden_size=None, latent_size=512,
        ...
        for i in range(self.n_branches):
            if multiclass_classes is not None and self.multiclass_classes[i] > 1:
                classifier_dict[str(i)] = MulticlassClassifier(hidden_size=hidden_size,
                                                               n_classes=self.multiclass_classes[i])
            else:
